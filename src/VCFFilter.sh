...
selected_individuals="individuals.tsv"
selected_individuals_txt="individuals.txt"
...
bed_file="chr1_windows.bed"
output_file="${VSC_DATA}/filtered.vcf.gz"
finaloutput_file="${VSC_DATA}/finaloutput.vcf.gz"

echo "filter VCF file using BED file"
# Filter VCF using BED file and redirect output to gzip-compressed VCF file
bedtools intersect -a "$vcf_file" -b "$bed_file" -header | gzip -c > "$output_file"
...
bcftools view -S "$selected_individuals_txt" "$output_file" | gzip "$finaloutput_file"
echo "VCF file filtered further based on selected individuals"
echo "Final output VCF file compressed using gzip"
